...
impl TickSerde {
    ...
    D: Deserializer<'de>,
  {
    ...
    let chars: TickType = chars
      .try_into()
      .map_err(|_| de::Error::custom("invalid tick length"))?;
...
mod tests {
  ...
  fn test_tick_deserialize() {
    ...
    assert_eq!(
      serde_json::from_str::<Test>(json_str).unwrap(),
      Test {
        tick: ['a', 'b', '1', 'o']
      }
    );
  ...
  fn test_tick_deserialize_char_validation() {
    // valid chars
    assert_eq!(
      serde_json::from_str::<Test>(r##"{"tick":"ab1o"}"##).unwrap(),
      Test {
        tick: ['a', 'b', '1', 'o']
      }
    );
    ...
    assert_eq!(
      serde_json::from_str::<Test>(r##"{"tick":";b1o"}"##).unwrap(),
      Test {
        tick: [';', 'b', '1', 'o']
      }
    );
    assert_eq!(
      serde_json::from_str::<Test>(r##"{"tick":";b1A"}"##).unwrap(),
      Test {
        tick: [';', 'b', '1', 'A']
      }
    );
    assert_eq!(
      serde_json::from_str::<Test>(r##"{"tick":";b1😀"}"##).unwrap(),
      Test {
        tick: [';', 'b', '1', '😀']
      }
    );
