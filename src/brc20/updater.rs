...
impl<'a, 'db, 'tx, L: LedgerReadWrite> BRC20Updater<'a, 'db, 'tx, L> {
  ...
  fn process_deploy(
    ...
    if let Some(stored_tick_info) = self
      .ledger
      .get_token_info(&tick)
      .map_err(|e| Error::LedgerError(e))?
    {
  ...
  fn process_mint(
    ...
    self
      .ledger
      .update_mint_token_info(&tick, minted, block_number)
      .map_err(|e| Error::LedgerError(e))?;
  ...
  fn process_transfer(
    ...
    let mut from_balance = self
      .ledger
      .get_balance(&from_script_key, &tick)
      .map_err(|e| Error::LedgerError(e))?
      .map_or(Balance::new(&tick), |v| v);
    ...
    self
      .ledger
      .update_token_balance(&from_script_key, from_balance)
      .map_err(|e| Error::LedgerError(e))?;
    ...
    self
      .ledger
      .remove_transferable(&from_script_key, &tick, inscription_id)
      .map_err(|e| Error::LedgerError(e))?;
