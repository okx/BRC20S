...
pub fn validate_pool_str(s: &str) -> Result<(),BRC30Error> {
  if s.len() != PID_BYTE_COUNT {
    return Err(BRC30Error::InvalidPoolId(s.to_string(),"pool id length is not 13".to_string()));
  }

  if !s.contains("#") {
    return Err(BRC30Error::InvalidPoolId(s.to_string(),"pool id must contains '#'".to_string()));
  }
  ...
  if suffix.is_err() {
    return Err(BRC30Error::InvalidPoolId(s.to_string(),"the suffix of pool id is not hex".to_string()));
  }
  Ok(())
}
