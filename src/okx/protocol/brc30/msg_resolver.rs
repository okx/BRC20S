...
impl BRC30Message {
  pub(crate) fn resolve<'a, O: OrdDataStoreReadOnly, M: BRC30DataStoreReadOnly>(
    client: &Client,
    ord_store: &'a O,
    brc30_store: &'a M,
...
mod tests {
  ...
  use crate::okx::datastore::{
    brc30::{redb::BRC30DataStore, BRC30DataStoreReadWrite, BRC30Tick, TickId, TransferInfo},
    ord::OrdDbReadWriter,
  };
  ...
  fn test_invalid_protocol() {
    ...
    let brc30_store = BRC30DataStore::new(&wtx);
    ...
    brc30_store
      .insert_inscribe_transfer_inscription(
        ...
        TransferInfo {
          ...
          tick_name: BRC30Tick::from_str("ordi").unwrap(),
          amt: 100,
        },
      )
      .unwrap();
    let _msg = BRC30Message {
      txid: op.txid,
      inscription_id: op.inscription_id,
      old_satpoint: op.old_satpoint,
      new_satpoint: op.new_satpoint,
      commit_input_satpoint: None,
      op: BRC30Operation::Transfer(Transfer {
