...
use crate::okx::datastore::btc::BTCError;
use bigdecimal::{
...
impl Num {
  // TODO check overflow
  pub fn checked_add(&self, other: &Num) -> Result<Self, BTCError> {
    Ok(Self(self.0.clone() + &other.0))
  }

  pub fn checked_sub(&self, other: &Num) -> Result<Self, BTCError> {
    if self.0 < other.0 {
      return Err(BTCError::Overflow {
  ...
  // TODO check overflow
  pub fn checked_mul(&self, other: &Num) -> Result<Self, BTCError> {
    Ok(Self(self.0.clone() * &other.0))
  }

  pub fn checked_powu(&self, exp: u64) -> Result<Self, BTCError> {
    match exp {
  ...
  pub fn checked_to_u8(&self) -> Result<u8, BTCError> {
    if !self.0.is_integer() {
      return Err(BTCError::InvalidInteger(self.clone().to_string()));
    }
    self.0.clone().to_u8().ok_or(BTCError::Overflow {
  ...
  pub fn checked_to_u64(&self) -> Result<u64, BTCError> {
    if !self.0.is_integer() {
      return Err(BTCError::InvalidInteger(self.clone().to_string()));
    }
    self
      .0
      .to_bigint()
      .ok_or(BTCError::InternalError(format!(
        "convert {} to bigint failed",
        self.0
      )))?
      .to_u64()
      .ok_or(BTCError::Overflow {
        op: String::from("to_u64"),
        org: self.clone().to_string(),
        other: Self(BigDecimal::from(BigInt::from(u64::MAX))).to_string(), // TODO: change overflow error to others
      })
  }

  pub fn checked_to_u128(&self) -> Result<u128, BTCError> {
    if !self.0.is_integer() {
      return Err(BTCError::InvalidInteger(self.clone().to_string()));
    }
    self
...
impl FromStr for Num {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    if s.starts_with('.') || s.ends_with('.') || s.find(['e', 'E', '+', '-']).is_some() {
      return Err(BTCError::InvalidNum(s.to_string()));
    }
    let num = BigDecimal::from_str(s).map_err(|_| BTCError::InvalidNum(s.to_string()))?;
...
mod tests {
  ...
  #[test]
  fn test_to_u64() {
    let n = Num::from_str(&format!("{}", u64::MAX)).unwrap();
    assert_eq!(n.checked_to_u64().unwrap(), u64::MAX);

    let n = Num::from_str("0").unwrap();
    assert_eq!(n.checked_to_u64().unwrap(), 0);

    let n = Num::from_str(&format!("{}{}", u64::MAX, 1)).unwrap();
    assert_eq!(
      n.checked_to_u64().unwrap_err(),
      BTCError::Overflow {
        op: String::from("to_u64"),
        org: n.to_string(),
        other: Num::from(u64::MAX).to_string(),
      }
    );

    let n = Num::from_str(&format!("{}.{}", u64::MAX - 1, "33333")).unwrap();
    assert_eq!(
      n.checked_to_u64().unwrap_err(),
      BTCError::InvalidInteger(n.to_string())
    );
    ...
    assert_eq!(a.to_u64().unwrap(), 0_u64);

    let n = Num::from_str("3140000000000000000.00").unwrap();
    assert_eq!(n.checked_to_u64().unwrap(), 3140000000000000000u64);

    let n = Num::from_str(&format!("{}.{}", u64::MAX - 1, "33333")).unwrap();
    assert_eq!(n.scale(), 5_i64);
    assert_eq!(
      Num::from_str("1e2").unwrap_err(),
      BTCError::InvalidNum("1e2".to_string())
    );
    assert_eq!(
      Num::from_str("0e2").unwrap_err(),
      BTCError::InvalidNum("0e2".to_string())
    );

    assert_eq!(
      Num::from_str("100E2").unwrap_err(),
      BTCError::InvalidNum("100E2".to_string())
    );
  }
