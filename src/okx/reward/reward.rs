...
pub fn withdraw_user_reward(
  ...
  println!("withdraw_user_reward in");
  ...
  if user_staked <= Num::zero() {
    println!("withdraw_user_reward out");
...
pub fn update_user_stake(
  ...
  if pool.ptype == PoolType::Fixed {
    ...
    println!("update_user_stake out");
    return Err(BRC30Error::UnknownPoolType);
  }

  user.latest_updated_block = pool.last_update_block;

  println!("  reward_debt:{}", user.reward_debt.clone());
...
mod tests {
  ...
  fn test_fix_one_user() {
    ...
    do_one_case(
      ...
      0,
      0,
      Err(BRC30Error::NoStaked(pid.to_lowercase().hex())),
      Ok(()),
    );
    // case-2-A deposit 1
    do_one_case(
